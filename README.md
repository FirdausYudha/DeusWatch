...
- **Extend** - support **new log sources without code** via **[custom decoders](docs/features/11-decoders.md)** (data-driven regex → fields, live-reloaded, with an in-UI tester on your own raw lines), ingest **Suricata / Emerging Threats (ET Open/Pro)** network-IDS alerts as a first-class source (see [docs/suricata.md](docs/suricata.md)), and receive **[Wazuh manager alerts](docs/wazuh-webhook.md)** over an HTTP webhook - the rich Wazuh JSON (source IP, MITRE, severity, FIM `syscheck` + who-data) is mapped straight to the dashboard and enriched on top.
- **Enrich & score** - source IPs scored with CTI (AbuseIPDB, AlienVault OTX) and GeoIP; **country resolved from AbuseIPDB → OTX → GeoIP** and severity escalates automatically on high-confidence threats. Lookups are cached with a **UI-configurable dedup window**; no key configured shows an honest "-", never fabricated data. A **composite threat score (0-100) per source IP** (Multi-Source Event Correlation) accumulates `fired_times + AbuseIPDB + OTX + worst severity` over a window and shows as a small colored **doughnut** on every Events/Alerts row. Optional **LLM analysis** (provider-agnostic: Claude, Ollama, or any OpenAI-compatible endpoint) powers AI report summaries (on-demand + scheduled), with opt-in per-alert triage.
- **Respond (SOAR)** - a **progressive ban** engine: repeat offenders escalate down a configurable duration ladder (e.g. `10m → 30m → 1h → 24h → permanent`), all editable from the UI. Supports **automatic banning**, an **observation window**, an **IP whitelist**, per-offender **dedup**, a **per-IP response view**, **bulk approve/dismiss**, **unban**, and a **scenario ban** that fires when an IP's composite score crosses a threshold. Enforcement via nftables (agent-side), CrowdSec LAPI, or **MikroTik with multi-endpoint sync** - a ban/unban propagates to **every** connected router within ~10s (CrowdSec-bouncer style) and self-heals after a reboot. Beyond IP bans, **[network containment](docs/features/10-network-containment.md)** isolates a *compromised host* from the LAN when a rule authorizes it, a **trusted-session gate** suppresses file-change alerts from a whitelisted admin/deploy login, and every alert carries a **[remediation playbook](docs/features/12-playbooks.md)**.
- **Superior FIM** - file integrity monitoring that goes beyond "a file changed": a **content diff** shows exactly which lines were added/removed (the code injected during a defacement), **file-hash reputation** (CIRCL/VirusTotal) flags known-bad files, who-data (from a Wazuh feed) attributes the change to a process, and **one-click restore** reverts a defaced/modified file to its known-good snapshot on the endpoint - all in [docs/features/05-agents.md](docs/features/05-agents.md).
...
- **Self-monitoring** - the system notices its own failures: agents have live health states (`online → degraded → disconnected → stale`) and **an agent that stops reporting raises a high-severity alert** through the same pipeline as attacks (a dead agent can mean an attacker silencing it - MITRE T1562.001). A **disk-watermark janitor** drops the oldest event chunks before PostgreSQL can run out of disk, alerting on every trigger; all services expose `/healthz` + `/readyz`.
...
| **Phase 6.6** | **Wazuh manager ingest** (rich JSON + FIM syscheck + who-data), **MikroTik multi-endpoint blocklist sync** (CrowdSec-bouncer style, ~10s propagation + self-heal), **composite threat scoring** per source IP + **scenario ban** + dashboard doughnut, **superior FIM** (content diff + one-click restore), "PHP wrote PHP into webroot" behaviour rule | ✅ implemented · scoring/sync/FIM live- & mock-verified |
| Phase 7 | Linux process audit (auditd/sysmon), YARA scan on changed files, real-time FIM (fsnotify), native syslog input, OpenSearch pull, rule/integration marketplace, Helm chart | planned |
...
| 🛡️ Firewall | **MikroTik RouterOS** | Pushes blocked IPs to a RouterOS address-list via the REST API. Add **several** routers - a ban/unban syncs to all within ~10s and self-heals after a reboot. |
