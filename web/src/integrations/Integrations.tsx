  ...
  return (
    <label className="block">
      ...
      {field.help && !(field.secret && configured) && (
        <span className="mt-1 block text-[12.5px] text-dim">{field.help}</span>
      )}
    </label>
  )
}
  ...
  return (
    <section className="mb-8 rounded-[12px] border border-border bg-surface p-5">
      <div className="flex items-center gap-2">
        <h2 className="text-[13.5px] font-semibold text-fg">Log ingest webhook (Wazuh & others)</h2>
        <span className="rounded px-1.5 py-0.5 text-[11px] font-medium text-cyan-300 bg-cyan-500/15">Inbound</span>
        <span className={`rounded px-1.5 py-0.5 text-[11px] font-medium ${enabled ? 'text-emerald-300 bg-emerald-500/15' : 'text-muted bg-surface-2'}`}>
        ...
        <>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={disable} disabled={busy}
              className="rounded-[8px] border border-rose-900/60 px-3 py-2 text-[13.5px] text-rose-300 hover:bg-rose-500/10 disabled:opacity-50">
              Disable
            </button>
          </div>
...
export default function Integrations() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-5">
      ...
      {/* Add integration */}
      <section className="mb-8 rounded-[12px] border border-border bg-surface p-5">
        <h2 className="mb-4 text-[12.5px] font-semibold uppercase tracking-wider text-dim">Add integration</h2>
        <div className="flex flex-wrap gap-2">
          {types.map((t) => (
            <button
              ...
              className={`rounded-[8px] border px-3 py-2 text-left text-[13.5px] transition-colors ${
                pick === t.type
                  ? 'border-accent bg-accent-soft text-accent'
                  : 'border-border bg-surface-2 text-fg hover:bg-surface-2'
              }`}
            >
              <span className={`mr-2 rounded px-1.5 py-0.5 text-[11px] font-medium ${CATEGORY_BADGE[t.category]}`}>
        ...
        {picked && (
          <div className="mt-5 rounded-[8px] border border-border bg-surface p-4">
            <div className="mb-3 flex items-start justify-between gap-3">
              <p className="text-[13.5px] text-muted">{picked.desc}</p>
              {picked.doc && <DocLink file={picked.doc} className="shrink-0" />}
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="mb-1 block text-[12.5px] font-medium text-muted">Name</span>
                <input
                  value={name}
      ...
      <section>
        <h2 className="mb-3 text-[12.5px] font-semibold uppercase tracking-wider text-dim">
                ...
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {t && (
                        <span className={`rounded px-1.5 py-0.5 text-[11px] font-medium ${CATEGORY_BADGE[t.category]}`}>
                          {CATEGORY[t.category] ?? t.category}
                        </span>
                      )}
                      {!it.enabled && (
                        <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[11px] text-muted">disabled</span>
                      )}
                    </div>
                    <div className="mt-0.5 truncate text-[12.5px] text-dim">{t?.label ?? it.type}</div>
                  </div>
      ...
      {editing && editType && (
        <div className="fixed inset-0 z-20 grid place-items-center bg-black/50 p-4" onClick={() => setEditing(null)}>
          ...
          >
            ...
            <label className="mt-4 flex items-center gap-2 text-[13.5px] text-fg">
              <input
                type="checkbox"
                checked={editEnabled}
