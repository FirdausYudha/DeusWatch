  ...
  return (
    <div className="grid gap-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Label * <span className="text-dim">(the alert's deuswatch.label this playbook applies to)</span></span>
      ...
      <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Steps * <span className="text-dim">(one per line, in order - what the analyst should do)</span></span>
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-5">
      ...
      <section className="mb-8 rounded-[12px] border border-border bg-surface p-5">
        <h2 className="mb-4 text-[12.5px] font-semibold uppercase tracking-wider text-dim">Add playbook</h2>
      ...
      <section>
        <h2 className="mb-3 text-[12.5px] font-semibold uppercase tracking-wider text-dim">Catalog ({items.length})</h2>
        {items.length === 0 ? (
            ...
            {items.map((p) => (
              <div key={p.id} className="rounded-[12px] border border-border bg-surface p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {p.builtin && <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[11px] text-muted">builtin</span>}
                  <span className={`rounded px-1.5 py-0.5 text-[12.5px] ${p.enabled ? 'bg-emerald-500/15 text-emerald-300' : 'bg-surface-2 text-muted'}`}>
                    {p.enabled ? 'enabled' : 'disabled'}
                  </span>
                  <div className="ml-auto flex gap-2">
                    <button onClick={() => toggle(p)} className="rounded-md border border-border px-2 py-1 text-[12.5px] text-fg hover:bg-surface-2">{p.enabled ? 'Disable' : 'Enable'}</button>
                    <button onClick={() => { setEditing(p); setEditSpec(specOf(p)) }} className="rounded-md border border-border px-2 py-1 text-[12.5px] text-fg hover:bg-surface-2">Edit</button>
                    <button onClick={() => remove(p)} className="rounded-md border border-rose-900/60 px-2 py-1 text-[12.5px] text-rose-300 hover:bg-rose-500/10">Delete</button>
                  </div>
                </div>
                <ol className="mt-3 list-decimal space-y-1 pl-5 text-[13.5px] text-muted">
      ...
      {editing && (
        <div className="fixed inset-0 z-20 grid place-items-center bg-black/50 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-2xl rounded-[12px] border border-border bg-surface p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="mb-4 text-[13.5px] font-semibold text-fg">Edit playbook — <span className="text-accent">{editing.label}</span></h3>
