...
const input = 'w-full rounded-[8px] border border-border bg-surface-2 px-3 py-2 text-[13.5px] text-fg outline-none focus:border-accent'
...
function Form({ value, onChange }: { value: DecoderSpec; onChange: (v: DecoderSpec) => void }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Name</span>
        <input className={input} value={value.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. haproxy-http" /></label>
      <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Dataset *</span>
      ...
      <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Category</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Action</span>
          <input className={input} value={value.action} onChange={(e) => set('action', e.target.value)} /></label>
        <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Outcome</span>
          <input className={input} value={value.outcome} onChange={(e) => set('outcome', e.target.value)} placeholder="failure" /></label>
        <label className="block"><span className="mb-1 block text-[12.5px] text-muted">Level</span>
      ...
      <label className="block sm:col-span-2"><span className="mb-1 block text-[12.5px] text-muted">Regex (Go RE2, named groups -&gt; fields) *</span>
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-5">
      ...
      <section className="mb-8 rounded-[12px] border border-border bg-surface p-5">
        <h2 className="mb-4 text-[12.5px] font-semibold uppercase tracking-wider text-dim">Add decoder</h2>
        ...
        <div className="mt-5 border-t border-border pt-4">
          <div className="mb-2 flex items-center gap-3">
            <h3 className="text-[12.5px] font-semibold uppercase tracking-wider text-dim">Test against real log lines</h3>
            <button onClick={loadSamples} className="rounded-md border border-border px-2 py-1 text-[12.5px] text-fg hover:bg-surface-2">
            ...
            <div className="mb-3 max-h-40 overflow-auto rounded-[8px] border border-border bg-bg p-2">
              {samples.map((l, i) => (
                <button key={i} onClick={() => { setTestLine(l); runTest(l) }}
                  className="block w-full truncate px-2 py-1 text-left font-mono text-[12.5px] text-muted hover:bg-surface-2 hover:text-fg" title={l}>
                  {l}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => runTest(testLine)} className="shrink-0 rounded-[8px] border border-border px-3 py-2 text-[13.5px] text-fg hover:bg-surface-2">Test</button>
          </div>
          {testMsg && <p className="mt-2 text-[12.5px] text-amber-400">{testMsg}</p>}
          {testResult && (
                    ...
                    <table className="mt-2 text-xs">
                      <tbody>
                        {Object.entries(testResult.fields).map(([k, v]) => (
                          <tr key={k}><td className="pr-3 font-mono text-dim">{k}</td><td className="font-mono text-fg">{v}</td></tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <section>
        <h2 className="mb-3 text-[12.5px] font-semibold uppercase tracking-wider text-dim">Configured ({items.length})</h2>
        {items.length === 0 ? (
          <p className="rounded-[12px] border border-dashed border-border px-4 py-8 text-center text-[13.5px] text-dim">
              ...
              <tbody className="divide-y divide-border bg-surface">
                {items.map((d) => (
                  <tr key={d.id} className="hover:bg-surface-2">
                    ...
                    <td className="px-4 py-2 max-w-[16rem] truncate font-mono text-[12.5px] text-dim" title={d.regex}>{d.regex}</td>
                    <td className="px-4 py-2">
                      <span className={`rounded px-1.5 py-0.5 text-[12.5px] ${d.enabled ? 'bg-emerald-500/15 text-emerald-300' : 'bg-surface-2 text-muted'}`}>
                        {d.enabled ? 'enabled' : 'disabled'}
                      </span>
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => toggle(d)} className="rounded-md border border-border px-2 py-1 text-[12.5px] text-fg hover:bg-surface-2">{d.enabled ? 'Disable' : 'Enable'}</button>
                        <button onClick={() => { setEditing(d); setEditSpec(specOf(d)) }} className="rounded-md border border-border px-2 py-1 text-[12.5px] text-fg hover:bg-surface-2">Edit</button>
                        <button onClick={() => remove(d)} className="rounded-md border border-rose-900/60 px-2 py-1 text-[12.5px] text-rose-300 hover:bg-rose-500/10">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {editing && (
        <div className="fixed inset-0 z-20 grid place-items-center bg-black/50 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-2xl rounded-[12px] border border-border bg-surface p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="mb-4 text-[13.5px] font-semibold text-fg">Edit decoder — <span className="text-accent">{editing.name}</span></h3>
            <Form value={editSpec} onChange={setEditSpec} />
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => setEditing(null)} className="rounded-[8px] border border-border px-4 py-2 text-[13.5px] text-fg hover:bg-surface-2">Cancel</button>
