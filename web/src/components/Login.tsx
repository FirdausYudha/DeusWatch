    ...
    <div className="grid h-screen place-items-center bg-bg text-fg">
      <form onSubmit={submit} className="w-80 space-y-5 rounded-2xl border border-border bg-surface p-6 shadow-xl">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <div className="text-[12.5px] text-dim">{isRegister ? 'Create a new account' : 'Sign in to continue'}</div>
        </div>
        ...
        <div className="space-y-3">
          <input
            ...
            autoFocus
            disabled={need2fa}
            className="w-full rounded-[8px] border border-border bg-surface-2 px-3 py-2 text-[13.5px] outline-none focus:border-accent disabled:opacity-60"
          />
          <input
            type="password"
            value={password}
          ...
          {need2fa && (
            <input
              ...
              inputMode="numeric"
              autoFocus
              className="w-full rounded-[8px] border border-indigo-700 bg-surface-2 px-3 py-2 text-[13.5px] tracking-widest outline-none focus:border-accent"
            />
          )}
        </div>
        ...
        <button
          type="submit"
          disabled={!canSubmit}
          className="w-full rounded-[8px] bg-accent py-2 text-[13.5px] font-medium text-white transition-colors hover:opacity-90 disabled:opacity-50"
        >
          {busy ? 'Processing…' : need2fa ? 'Verify' : isRegister ? 'Sign up' : 'Sign in'}
        </button>

        {isRegister && (
          <p className="text-center text-[12.5px] text-dim">New accounts get the viewer role (read-only).</p>
        )}
      </form>
    </div>
  )
