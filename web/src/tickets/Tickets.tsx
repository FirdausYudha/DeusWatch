            ...
            {tickets.map((t) => (
              <tr key={t.id} className="cursor-pointer hover:bg-surface-2" onClick={() => setSelectedId(t.id)}>
                ...
                <td className="px-4 py-2">
                  <span className={`rounded px-1.5 py-0.5 text-[12.5px] font-medium ${STATUS_BADGE[t.status]}`}>
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-[12px] border border-border bg-surface p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="mb-4 text-[16px] font-semibold text-fg">New ticket</h2>
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <label className="flex-1">
              <span className="mb-1 block text-[12.5px] font-medium text-muted">Severity</span>
              <select
            ...
            <label className="flex-1">
              <span className="mb-1 block text-[12.5px] font-medium text-muted">Assignee (optional)</span>
              <input
        ...
        {!ticket ? (
          ...
          <>
            <div className="mb-3 flex items-start justify-between gap-4">
              <h2 className="text-[16px] font-semibold text-fg">{ticket.title}</h2>
            ...
            {ticket.description && (
              <pre className="mb-4 whitespace-pre-wrap rounded-[8px] border border-border bg-bg p-3 text-[12.5px] text-fg">
                {ticket.description}
              </pre>
            )}
            ...
            {canManage && (
              <div className="mb-5 space-y-3 rounded-[8px] border border-border bg-surface p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[12.5px] font-medium text-dim">Status:</span>
                  {STATUSES.map((s) => (
                    <button
                      key={s}
                      onClick={() => patch({ status: s })}
                      disabled={ticket.status === s}
                      className={`rounded-md border px-2 py-1 text-[12.5px] transition-colors ${
                        ticket.status === s
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[12.5px] font-medium text-dim">Assignee:</span>
                  <span className="text-[12.5px] text-fg">{ticket.assignee || 'unassigned'}</span>
                  ...
                  <span className="ml-2 text-[12.5px] font-medium text-dim">Severity:</span>
                  <select
                    value={ticket.severity}
                    onChange={(e) => patch({ severity: Number(e.target.value) })}
                    className="rounded-md border border-border bg-surface-2 px-2 py-1 text-[12.5px] outline-none focus:border-accent"
                  >
                    {[0, 1, 2, 3, 4].map((s) => (
                      <option key={s} value={s}>
                        {SEVERITY[s].label}
            ...
            {/* Case notes */}
            <h3 className="mb-2 text-[12.5px] font-semibold uppercase tracking-wider text-dim">Case notes ({comments.length})</h3>
            <div className="space-y-2">
              {comments.length === 0 && <p className="text-[12.5px] text-dim">No notes yet.</p>}
              {comments.map((c) => (
                <div key={c.id} className="rounded-[8px] border border-border bg-surface p-3">
                  ...
                  <p className="whitespace-pre-wrap text-[13.5px] text-fg">{c.body}</p>
                </div>
              ))}
            </div>
            ...
            {canManage && (
              <div className="mt-3 flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-[8px] border border-border bg-surface-2 px-3 py-2 text-[13.5px] outline-none focus:border-accent"
                />
                <button
                  onClick={postComment}
